window.__require = function e(t, n, r) {
  ...
  ClientTest: [ function(require, module, exports) {
    ...
    exports.Gate = exports.dir = exports.moveChess = void 0;
    ...
    var ClientTest = function(_super) {
      ...
      ClientTest.prototype.playDiceAnimation = function() {
        ...
        this.schedule(function() {
          _this.Dice.getComponent(cc.Sprite).spriteFrame = cc.find("dice" + Dicepoint.toString(), _this.Dice).getComponent(cc.Sprite).spriteFrame;
        }, 1);
      };
      ...
      NewClass.prototype.GetAItosever = function() {
        ...
        WebSocket_1.default.send(JSON.stringify(msg), function(data) {
          ...
          for (var i = 0; i < 4; i++) {
            ...
            for (var j = 0; j < 10; j++) cc.find(i.toString() + "/view/content/" + (10 * i + j).toString(), _this.ChooseAI).active = false;
          }
          for (var i = 0; i < 4; i++) for (var j = 0; j < data["AINames"].length; j++) {
            cc.find(i.toString() + "/view/content/" + (10 * i + j).toString(), _this.ChooseAI).active = true;
            cc.find(i.toString() + "/view/content/" + (10 * i + j).toString(), _this.ChooseAI).on("click", _this.SetAI, _this);
            cc.find(i.toString() + "/view/content/" + (10 * i + j).toString() + "/Background/Label", _this.ChooseAI).getComponent(cc.Label).string = data["AINames"][j];
          }
        });
      };
      NewClass.prototype.SetAI = function(btn) {
        Ai[Math.floor(parseInt(btn.name) / 10)] = AiNames[Math.floor(parseInt(btn.name) % 10)];
        0 == Math.floor(parseInt(btn.name) / 10) ? this.name0.string = AiNames[Math.floor(parseInt(btn.name) % 10)] : 1 == Math.floor(parseInt(btn.name) / 10) ? this.name1.string = AiNames[Math.floor(parseInt(btn.name) % 10)] : 2 == Math.floor(parseInt(btn.name) / 10) ? this.name2.string = AiNames[Math.floor(parseInt(btn.name) % 10)] : 3 == Math.floor(parseInt(btn.name) / 10) && (this.name3.string = AiNames[Math.floor(parseInt(btn.name) % 10)]);
      };
